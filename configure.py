...
def get_venv_server_path() -> Path:
    """Get the path to the installed console script in the virtual environment."""
    script_dir = Path(__file__).parent
    venv_dir = script_dir / ".venv"

    if platform.system() == "Windows":
        python_path = venv_dir / "Scripts" / "odoo-mcp-server.exe"
    else:
        python_path = venv_dir / "bin" / "odoo-mcp-server"
...
def generate_config(credentials: dict[str, str]) -> dict:
    ...
    python_path = get_venv_server_path()
